...
export const occupantQueries = (req, res) => {
  ...
  db.sequelize
    ...
    .then((results) =>
      res.json({ success: true, id: results[0][0].occupant_insert })
    )
...
export const getOccupants = (req, res) => {
  ...
  db.sequelize
    .query(`SELECT * FROM public.get_occupants ('${query_type}','${id}')`)
...
export const parcelQueries = (req, res) => {
  ...
  let sql = `SELECT * FROM public.__parcel_insert_fresh('${query_type}','${state}','${
    district ? district.replace(/'/g, "’") : ""
  }','${lga}','${ward ? ward.replace(/'/g, "’") : ""}','${
    address ? address.replace(/'/g, "’") : ""
  }','${property_id_no}','${block_no}','${plot_no}','${
    street_name ? street_name.replace(/'/g, "’") : ""
  }','${
    owner_name ? owner_name.replace(/'/g, "’") : ""
  }','${owner_type}','${owner_geder}','${telephone1}','${telephone2}','${occupancy_type}','${any_buildings}','${main_use}','${parcel_fenced}','${size_in_m2}','${doc_type}','${water}','${sewerage}','${electricity}','${street_lights}','${waste_disposal}','${geom}','${id}')`;
  ...
  db.sequelize
    ...
    .then((results) => {
      ...
      // }else{
      res.json({ success: true, id: results[0] });
    })
    .catch((error) => res.status(500).json({ success: false, error }));
};
...
export const getParcels = (req, res) => {
  const { query_type, id } = req.params;
  db.sequelize
    .query(`SELECT * FROM public.get_parcels('${query_type}','${id}')`)
    // ,{replacements: {id,query_type}  })
    .then((results) => res.json({ success: true, data: results[0] }))
    .catch((error) => res.status(500).json({ success: false, error }));
};

export const getApiParcels = (req, res) => {
  const { id = null, action = null, keyword = null } = req.query;
  db.sequelize
    .query(`SELECT * FROM public.___get_parcels(:action,:id)`, {
      replacements: {
        action,
        id: id ? id : keyword,
      },
...
export const structureQueries = (req, res) => {
  ...
  db.sequelize
    .query(
      `SELECT * FROM public.__structure_insert(:query_type,:finished,:level_of_completion,:year_completed,:main_use,:residential_type,:wall_material,:roof_covering,:roof_type,:no_floors,:no_of_occupants,:property_id_no,:parcel_id,:shape_length,:shape_area,:parent_id,:id,:geom);`,
      {
        replacements: {
          query_type,
          finished,
          ...
          no_floors,
          no_of_occupants,
          property_id_no: property_id_no ? property_id_no.toString() : "",
          parcel_id: parcel_id ? parcel_id.toString() : "",
          shape_length,
          shape_area,
    ...
    .then((results) =>
      res.json({ success: true, results: results[0][0].structure_insert })
    )
    .catch((error) => res.status(500).json({ success: false, error }));
};
...
export const getStructures = (req, res) => {
  const { query_type, id } = req.params;
  db.sequelize
    .query(`SELECT * FROM public.__get_structures ('${query_type}','${id}')`)
    .then((results) => res.json({ success: true, data: results[0] }))
    .catch((error) => res.status(500).json({ success: false, error }));
};
export const savePolygon = (req, res) => {
  ...
  db.sequelize
    .query(
      `SELECT * FROM  public.__polygon_queries('${query_type}','${file_type}','${id}','${structure_id}','${polygon}')`
    )
    .then((results) => res.json({ success: true, data: results[0] }))
    .catch((error) => res.status(500).json({ success: false, error }));
};
...
export const apiSummaryReport = (req, res) => {
